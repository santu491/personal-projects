...
import { appConfigInit } from '@anthem/communityadminapi/bootstrap';
import { APP, IConfig, getArgument } from '@anthem/communityadminapi/utils';
import { resolve } from 'path';
import { decrypt } from './loaders/decrypt';
import { getConfig, getDBConfig, getSecrets, getWordList } from './loaders/handleConfig';
...
export async function loadConfigs() {
  ...
  if (isLocal) {
    const result = await loadConfig(resolve('./app/adminresources/config/config.local.json'));
    return appConfigInit(result, null);
  }

  // Load the AWS secret localy with SAML setup Uncomment the below commented code line
  ...
  if (config !== null) {
    ...
    const dbDetails = getDBConfig(secret, isLocal);
