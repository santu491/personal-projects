import { HttpMethod, IHttpHeader, IHttpRequest, IHttpRequestInterceptor } from '@anthem/communityadminapi/http';
...
export class RestRequestInterceptor implements IHttpRequestInterceptor {
  transform(requestOptions: IHttpRequest): IHttpRequest {
    ...
    this.addHeader(requestOptions.headers, 'meta-ipaddress', RequestContext.getContextItem('clientIp') as string);
    this.addHeader(requestOptions.headers, 'meta-locale', RequestContext.getContextItem('locale') as string);
