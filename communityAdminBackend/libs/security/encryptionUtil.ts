  ...
  static init() {
    if (!this._aesBouncy) {
      this._aesBouncy = new AesBouncyEncryptor(
        APP.config.security.encryption.bouncyEncryption.salt,
        APP.config.security.encryption.bouncyEncryption.secret,
        APP.config.security.encryption.bouncyEncryption.iv,
        1000,
        256
      );
    }
    if (!this._aesSymmetric) {
      this._aesSymmetric = new AesSymmetricEncryptor(
        this.getSalt(APP.config.security.encryption.storePath, this.getPassword(APP.config.security.encryption.storeSecret)),
        this.getPassword(APP.config.security.encryption.aesSymmetric.secret)
      );
    }

    if (!this._aes) {
      this._aes = new AesEncryptor2(APP.config.security.encryption.aes.salt, this.getPassword(APP.config.security.encryption.aes.secret), APP.config.security.encryption.aes.iv);
    }
  }
