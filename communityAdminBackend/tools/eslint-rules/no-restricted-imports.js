...
const FAILURE_STRING1 = 'import statement forbidden (use @anthem/communityadminapi/xyz package name)';
  ...
  create(constext) {
    ...
    function isRestrictedPattern(importSource, fileName, options, node) {
      ...
      if (options.libs && options.apis && importSource && importSource.indexOf('@') >= 0) {
        ...
        const rg5 = new RegExp('libs\\/' + importSource.replace('@anthem/communityadminapi/', '') + '\\/');
        let rg6 = new RegExp('api\\/' + importSource.replace('@', '') + '\\/');

        if (importSource.indexOf('/mocks') < 0 && rg2.test(importSource)) {
          if (importSource.replace('communityadminapi/', '').split('/').length !== 2) {
