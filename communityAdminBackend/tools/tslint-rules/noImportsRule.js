...
var Rule = (function(_super) {
  ...
  Rule.FAILURE_STRING1 = 'import statement forbidden (use @anthem/communityadminapi/xyz package name)';
  ...
  NoImportsWalker.prototype.checkForBannedImport = function(expression, sourceFile) {
    if (options) {
      ...
      if (options.libs && options.apis && tsutils_1.isTextualLiteral(expression) && expression.text.indexOf('@') >= 0) {
        ...
        var rg5 = new RegExp('libs\\/' + expression.text.replace('@anthem/communityadminapi/', '') + '\\/');
        ...
        if (expression.text.indexOf('/mocks') < 0 && rg2.test(expression.text)) {
          if ((expression.text.replace('communityadminapi/', '')).split('/').length !== 2) {
