...
import { mockProgramSvc, mockResult } from '@anthem/communityapi/common/baseTest';
import { mockILogger } from '@anthem/communityapi/logger/mocks/mockILogger';
import { ProgramController } from '../programController';

describe('ProgramController', () => {
  let ctrl: ProgramController;
  ...
  it('Verify Resp', async () => {
    ...
    mockProgramSvc.getServiceTags.mockReturnValue(expResp);
    const resp = await ctrl.getServiceTags();
    expect(resp).toBe(expResp);
  });

  it('Should Return Program details by Id', async () => {
    const programId: string = '5152119452598272';
    const expRes = {
      data: {
        isSuccess: true,
        isException: false,
        value: {
          ...
          attributeTags: ['children', 'low-income'],
          availability: 'available',
          coverageDescription:
            'This program covers residents of the following states: CA.',
          description:
            'Head Start is a federal program for preschool children three to five years of age in low-income families.',
          directions: '',
          ...
          offices: [
            {
              ...
              urlSafeKey:
                'ahJzfnNlYXJjaGJlcnRoYS1xYTFyEwsSBk9mZmljZRiAgMDVjNOACAw'
            }
          ],
    ...
    mockProgramSvc.getProgramById.mockReturnValue(expRes);
    const data = await ctrl.getProgram(programId, 'abcbs-in');
    expect(data).toEqual(expRes);
  });

  it('Should Return programs in a given zip code and limit to specific terms, attribute tags or service tags', async () => {
    const zipCode: number = 12345;
    const cursor: number = 0;
    const limit: number = 1;
    const terms: string = 'all';
          ...
          programs: [
            {
              ...
              coverage_description:
                'This program covers residents of the following states: NY.',
              description:
              ...
              offices: [
                {
                  ...
                  url_safe_key:
                    'ahJzfnNlYXJjaGJlcnRoYS1xYTFyEwsSBk9mZmljZRiAgID28umBCww'
                }
              ],
    ...
    mockProgramSvc.getProgramsListByZipCode.mockReturnValue(expRes);
    const data = await ctrl.getProgramsListByZipCode(
      zipCode,
      cursor,
      limit,
      terms,
  ...
  it('Should Return Program details by Id - error', async () => {
    const expRes = {
      data: {
        isSuccess: false,
        isException: false,
        errors: [
          {
            ...
            errorCode: 400,
            title: API_RESPONSE.messages.noProgramIdTitle,
            detail: API_RESPONSE.messages.noProviderIdDetail
          }
        ]
      }
    };
    mockResult.createError.mockReturnValue(expRes);
    const data = await ctrl.getProgram(undefined, 'abcbs-in');
    expect(data).toEqual(expRes);
  });

  it('Should Return programs in a given zip code and limit to specific terms, and undefined values', async () => {
    const zipCode: number = 12345;
    const cursor: number = 0;
    const limit: number = 1;
    const terms: string = 'all';
    const attributeTag: string = undefined;
    const atoperand: string = 'and';
    const serviceTag: string = '';
    const stoperand: string = undefined;
    const expRes = {
      data: {
        isSuccess: true,
        isException: false,
  ...
  it('Should Return programs in a given zip code and limit to specific terms, error', async () => {
    const zipCode: number = 12345;
    const cursor: number = 0;
    const limit: number = 1;
    const terms: string = undefined;
    const attributeTag: string = undefined;
    const atoperand: string = undefined;
    const serviceTag: string = undefined;
    const stoperand: string = undefined;
    const expRes = {
      data: {
        isSuccess: false,
        ...
        errors: [
          {
            id: '360f9591-37d1-5baf-76e3-aabf28f95ef9',
            errorCode: 400,
            title: API_RESPONSE.messages.noTermsOrTagsTitle,
            detail: API_RESPONSE.messages.noTermsOrTagsDetail
          }
        ]
      }
    };
