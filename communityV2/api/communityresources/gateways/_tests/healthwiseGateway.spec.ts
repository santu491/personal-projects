...
describe('healthWiseGateway', () => {
  ...
  it('Should Get Topic By Id: ES', () => {
    gateway.getTopicById('test', '5152119452598272','es');
    expect(mockRestClient.invoke.mock.calls.length).toBe(1);
  });
  ...
  it('Should Get Topic By ConceptId and AspectId', () => {
    gateway.getArticleTopic('test', 'HWCV_05501', 'selfCareTxOptions','es');
    expect(mockRestClient.invoke.mock.calls.length).toBe(1);
  });

  it('Should Get Article By Id', () => {
    gateway.getArticleById('test', 'HWCV_05501', 'es');
    expect(mockRestClient.invoke.mock.calls.length).toBe(1);
  });

  it('Should Get Article By Id', () => {
    gateway.getArticleById('test', 'HWCV_05501', 'en');
    expect(mockRestClient.invoke.mock.calls.length).toBe(1);
  });
});
