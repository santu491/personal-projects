...
import { BcryptHash, IJwtClaimConfigs, IJwtToken, JwtAlgorithms, JwtTokenUtil } from '@anthem/communityapi/security';
...
export class PublicJwtToken implements IJwtToken {
  ...
  async verify(token: string, claimConfig: IJwtClaimConfigs, url: string, id?: string, webguid?: string) {
    ...
    if (claims !== false && typeof claims !== 'boolean') {
      ...
      if (this.needAuthenticated(url) && claims.threatType === 'AUTHENTICATE' && claims.authInd === 'N') {
