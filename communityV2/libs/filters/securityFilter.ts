...
import { InputScanner } from '@anthem/communityapi/security';
...
export class SecurityFilter {
  ...
  private _inputScanner = new InputScanner(
    [],
    APP.config.security ? (APP.config.security.encodeProps) : [],
    APP.config.security ? (APP.config.security.passthroughProps) : [],
  ...
  scanUrlParams(urlParams: { [key: string]: string }) {
    ...
    for (const key in urlParams) {
      ...
      /* Commented to use Spanish as Url params
