import { SWAGGER_API_DOCS } from '@anthem/communityapi/common';
import { APP, getApiArgument, getArgument, getControllerPaths, getMiddlewarePaths, getResolverPaths, getSwaggerPaths, RequestContext } from '@anthem/communityapi/utils';
...
export const expressLoader: MicroframeworkLoader = async (settings: MicroframeworkSettings | undefined): Promise<void> => {
  if (settings) {
     ...
     *  @output api/communityresources/swagger/swagger.json
    ...
    fs.stat(swaggerFilePath, (e, stat) => {
      ...
      if (stat) {
        ...
        expressApp.use('/api-docs', swaggerUi.serve, swaggerUi.setup(swaggerFile, null));
      }
    });
