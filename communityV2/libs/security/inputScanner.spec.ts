...
describe('InputScanner UTest', () => {
  ...
  it('should sanitize properties by default for html', () => {
    ...
    expect(result.testSanitize).toBe('test<b>password</b>hello');
    expect(result.test.testSanitize).toBe('test<b>password</b>hello');
  });
  ...
  it('should throw error when charachter whitelist not adhered', () => {
    try {
      ...
      expect((e as Error).message.indexOf('Invalid whitelist characters on root.test.testWhitelist') >= 0).toBeTruthy();
    }
  ...
  it('should not encode, mask or sanitize passthorugh properties', () => {
    ...
    expect(result.testEncodeAll).toBe('test&lt;script&gt;password&lt;&#x2f;script&gt;hello');
    expect(result.test.testPassthrough).toBe('test<script>password</script>hello');
  });
  ...
  it('should encode or sanitize senstive properties with full property path defined', () => {
    ...
    expect(result.arr[0].testEncode).toBe('test&lt;script&gt;password&lt;&#x2f;script&gt;hello');
    expect(result.test.testEncode).toBe('testhello');
  });
  ...
  it('should encode senstive properties with property name defined', () => {
    ...
    expect(result.testEncodeAll).toBe('test&lt;script&gt;password&lt;&#x2f;script&gt;hello');
    expect(result.test.testEncodeAll).toBe('test&lt;script&gt;password&lt;&#x2f;script&gt;hello');
  });
  ...
  it('should mask senstive properties in url', () => {
    let result = scanner.scanUrlParams('http://www.anthem.com/api/test/123/test?test=123&testMaskAll=****');
    expect(result).toBe('http://www.anthem.com/api/test/123/test?test=123&testMaskAll=****');

    result = scanner.scanUrlParams('http://www.anthem.com/api/test/123/test?testMaskAll=456&test=123');
    expect(result).toBe('http://www.anthem.com/api/test/123/test?testMaskAll=****&test=123');
  });
});
