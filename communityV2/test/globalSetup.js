const { getArgument } = require('graphql/execution/values');
const { decrypt, utilsDecrypt } = require('../app/communityresources/loaders/decrypt');
const { dbDetails, DB_OPTIONS, collections, mongoDbTables } = require('../libs/common/constants');
...
const initDb = async () => {
  ...
  const endpoint = 'mongodb://' + envDBDetails.usrnm + ':' + utilsDecrypt(envDBDetails.dbAuth, 'aes-bouncy') + envDBDetails.endpoint;
